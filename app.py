...
def get_data():
    ...
    try:
        ...
        # ── Responsable: A=Banco, B=Correo, C=Responsable ──
        responsable = []
        rows_resp = read_excel_from_sp(SITE_COMMAND, RUTA_BD_RESP)
        for row in rows_resp[1:]:
            banco  = str(row[0] or '').strip()
            correo = str(row[1] or '').strip() if len(row) > 1 else ''
            nombre = str(row[2] or '').strip() if len(row) > 2 else ''
            if nombre:
                responsable.append({'nombre': nombre, 'correo': correo, 'banco': banco})
