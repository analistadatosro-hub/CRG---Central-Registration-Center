...
# ── BDs locales (carpeta public/) ──
BASE_LOCAL   = os.path.join(os.path.dirname(__file__), 'public')
LOCAL_CECO    = os.path.join(BASE_LOCAL, 'Bd_Ceco.xlsx')
LOCAL_ESTADO  = os.path.join(BASE_LOCAL, 'Bd_Estado_cliente.xlsx')
LOCAL_FAMILIA = os.path.join(BASE_LOCAL, 'Bd_Familia_y_Sub_familia.xlsx')
LOCAL_RESP    = os.path.join(BASE_LOCAL, 'Bd_Responsable.xlsx')
...
def get_data():
    try:
        ceco = {}
        for r in read_local(LOCAL_CECO)[1:]:
            c, v = s(r[0]), s(r[1])
            if c and v: ceco.setdefault(c, []).append(v)

        estado = {}
        for r in read_local(LOCAL_ESTADO)[1:]:
            c, v = s(r[0]), s(r[1])
            if c and v: estado.setdefault(c, []).append(v)

        familia = {}
        for r in read_local(LOCAL_FAMILIA)[1:]:
            c, f = s(r[0]), s(r[1])
        ...
        resp = []
        for r in read_local(LOCAL_RESP)[1:]:
            banco  = s(r[0])
    ...
    try:
        ...
        with write_lock:
            ...
            row = [
                ...
                '', '', '', '', '', '', '', '', '',  # 18-26: S→AA vacías
                'Pendiente'                # 27 AB - Estado Registro
            ]
