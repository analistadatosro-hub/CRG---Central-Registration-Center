...
def save_ticket():
    try:
        ...
        with write_lock:
            ...
            now = datetime.now()
            row = [
                d.get('wo', ''),           # col 0  → W.O / Ticket
                d.get('ceco', ''),          # col 1  → Ceco
                d.get('familia', ''),       # col 2  → Familia
                d.get('sub_familia', ''),   # col 3  → Sub Familia
                d.get('descripcion_ot', ''),# col 4  → Descripción OT / Estado cliente
                d.get('usuario', ''),       # col 5  → Usuario que creó la OT
                d.get('correo', ''),        # col 6  → Correo usuario
                d.get('detalle', ''),       # col 7  → Detalle
                d.get('tipo_ot', ''),       # col 8  → Tipo de Ticket (CBM/CBP)
                'RM',                       # col 9  → Tipo de OT (predeterminado)
                'ACK',                      # col 10 → Estado
                fmt_date(d.get('fecha_apertura')),  # col 11 → Fecha modif. estado
                d.get('usuario', ''),       # col 12 → Modificado por
                fmt_date(d.get('fecha_apertura')),  # col 13 → Fecha Apertura Cliente
                fmt_dt(now),               # col 14 → Fecha de Inicio Real
                '',                        # col 15 → Fecha de Cierre Real (vacío)
                d.get('prioridad', ''),    # col 16 → Prioridad (ALTA/MEDIA/BAJA)
                d.get('cliente', '')       # col 17 → Cliente (BCP/BBVA/BIF)
            ]
